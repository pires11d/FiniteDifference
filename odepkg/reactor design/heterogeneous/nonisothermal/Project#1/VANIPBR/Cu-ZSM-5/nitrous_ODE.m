...
% Energy Balance
dydz(5) = ((-dHr*pc*(1-eb)*ra)-(4*(U/D)*(T-Ta)))/(us*Cpg*pg);
% Catalyst Weight
dydz(7) = pc*(1-eb)*Ac;
%-------------------------------------------------------------------------%
...
ra2 = k*pc*(1-eb2)*Pa2/(1+K1*Pa2+K2*Pd2);
...
% Energy Balance (PBR)
dydz(6) = ((-dHr*pc*(1-eb2)*ra2)-(4*(U2/D2)*(T2-Ta)))/(us2*Cpg*pg2);
% Catalyst Weight (PBR)
dydz(8) = pc*(1-eb2)*Ac2;
%-------------------------------------------------------------------------%
