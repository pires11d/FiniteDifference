...
% k0    = kinetic rate pre-exponential factor for deactivation
% Ed    = deactivation process activation energy
% -------------------------------------------------------------------------
...
%
% phi   = (R^2*k0*exp(E/R*Ts))^(1/2)    % Thiele modulus for the main rxn
% phia  = (R^2*kd0*exp(Ed/R*Ts))^(1/2)  % Thiele modulus for deactivation
...
% Main function
%
function Pellet1ND % 1st order, Non-isothermal, with Deactivation
close;
...
c = [1; 1/Le];
f = [1; 1].*DuDx;
s = [-phi^2*u(1).*exp(1-1/u(2))*exp(-phia^2*t); ...
    beta*phi^2*u(1)*exp(gamma*(1-1/u(2)))*exp(-phia^2*t)];
% -------------------------------------------------------------------------
