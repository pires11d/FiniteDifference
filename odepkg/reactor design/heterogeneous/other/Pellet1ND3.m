...
% Main function
%
function Pellet1ND3 % 1st order, Non-isothermal, with Deactivation
close;
close all;
clear;
clearvars -global; % Clear previously stored global variables 
clc;
global u1 u2 u3 xmatrix tmatrix

tic % Computes CPU time, in seconds 

m = 2; % spherical coordinates
x = linspace(0,1,30); % We are taking 100 time points, from 0 to 1
t = linspace(0,1,30);  % We are taking  20 space points (radial), from 0 to 1

xmatrix = ones(size(t,2),size(x,2)); % Creates 2D matrix for x vector
for i=1:size(t,2)                    % required by interp2
    xmatrix(i,:) = x;
end

tmatrix = ones(size(t,2),size(x,2)); % Creates 2D matrix for t vector
for i=1:size(x,2)                    % required by interp2
    tmatrix(:,i) = t';               % Change t values to column
end
...
n_integral = 3*dblquad(@eta_integral,0,1,0,1);
eta = n_integral % Displays effectiveness factor value
...
% Activity equation by Natan Padoin
...
% Parameters for the N2O decomposition
...
%theta (u2)
%a (u3)
function z = eta_integral(qsi, tau)
% Function by Danilo Preve

global xmatrix tmatrix u1 u2 u3 gamma

psi = interp2(xmatrix,tmatrix,u1,qsi,tau,'spline');
theta = interp2(xmatrix,tmatrix,u2,qsi,tau,'spline');
a = interp2(xmatrix,tmatrix,u3,qsi,tau,'spline');

z = exp(gamma*(1-(1./theta))).*psi.*a.*(qsi.^2);
